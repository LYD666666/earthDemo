...
{  
    ...
    oper_type =  Tof_Data2.rx_buff[framh_index+4];/*获取操作类型*/
    
    if(oper_type == 0x00)/*读操作*/
    {
        
        data_len = Tof_Data2.rx_buff[framh_index+7];/*获取数据长度*/
        ...
        calchkval = CRC_Check_Sum(&Tof_Data2.rx_buff[framh_index],data_len+8);/*计算CRC校验和*/
        rxchkval = ((u16)Tof_Data2.rx_buff[framh_index+data_len+8]<<8) + Tof_Data2.rx_buff[framh_index+data_len+9] ; /*计算帧的CRC校验和*/
        
        if(calchkval == rxchkval)/*校验和正确*/
        {                          
              /*读取数据*/
              if(Tof_Data2.rx_buff[framh_index+7]==1)/*数据长度为1*/
                *data =  Tof_Data2.rx_buff[framh_index+8];
              else if(Tof_Data2.rx_buff[framh_index+7]==2)/*数据长度为2*/
    ...
    }else if(oper_type == 0x01)/*写操作*/
    {
         ...
         calchkval = CRC_Check_Sum(&Tof_Data2.rx_buff[framh_index],6);/*计算CRC校验和*/
         rxchkval = ((u16)Tof_Data2.rx_buff[framh_index+6]<<8) + Tof_Data2.rx_buff[framh_index+7] ; /*计算帧的CRC校验和*/
    ...
    }else if(oper_type == 0xff)/*异常操作*/
    {
        if((Tof_Data2.rx_buff[framh_index+2]==0XFF) && (Tof_Data2.rx_buff[framh_index+3]==0XFF))
        {
              ...
              //    printf("ERROR_CODE:%d\r\n",Tof_Data2.rx_buff[framh_index+5]); /*打印错误码*/
...
uint8_t Ms53l0m_RData_first(uint16_t addr,uint8_t reg,uint8_t datalen,uint16_t *data)
{
    ...
    Tof_Data2.tx_buff[0] = MFRAME_H;/*帧头*/
    Tof_Data2.tx_buff[1] = SENSOR_TYPE;/**/
    Tof_Data2.tx_buff[2] = (u8)(addr>>8);/*高位地址*/
    Tof_Data2.tx_buff[3] = (u8)(addr & 0XFF); /*低位地址*/
    Tof_Data2.tx_buff[4] = 0x00; /*读操作*/
    Tof_Data2.tx_buff[5] = reg; /*功能码*/
    Tof_Data2.tx_buff[6] = datalen; /*读取的长度*/
    
    calchkval =  CRC_Check_Sum(Tof_Data2.tx_buff,7);/*CRC校验*/
    
    Tof_Data2.tx_buff[7]  = (calchkval>>8) & 0xff;  /*CRC高位前*/
    Tof_Data2.tx_buff[8] =  (calchkval & 0xff); /*CRC低位后*/
...
//    printf("\r\n");
    ...
    Uart_Send_first(Tof_Data2.tx_buff,9);/*串口发送*/
...
uint8_t Ms53l0m_WData_first(uint16_t addr,uint8_t reg, uint8_t data)
{
    ...
    Tof_Data2.tx_buff[0] = MFRAME_H;/*帧头*/
    Tof_Data2.tx_buff[1] = SENSOR_TYPE;/*传感器类型2M TOF*/
    Tof_Data2.tx_buff[2] = (u8)(addr>>8);/*高位地址*/
    Tof_Data2.tx_buff[3] = (u8)(addr & 0XFF); /*低位地址*/
    Tof_Data2.tx_buff[4] = 0x01; /*写*/
    Tof_Data2.tx_buff[5] =  reg; /*功能码*/
    Tof_Data2.tx_buff[6] = 0x01;
    Tof_Data2.tx_buff[7] = data; /*数据*/
    
    calchkval =  CRC_Check_Sum(Tof_Data2.tx_buff,8);/*CRC校验*/
    
    Tof_Data2.tx_buff[8]  = (calchkval>>8) & 0xff;  /*CRC高位前*/
    Tof_Data2.tx_buff[9] =  (calchkval & 0xff); /*CRC低位后*/
...
//    printf("\r\n");
    ...
    Uart_Send_first(Tof_Data2.tx_buff,10);/*串口发送*/
...
void Ms53l0m_Init_first(void)
{
    ...
    Uart_Init_first(115200);/*串口配置115200*/
    
    delay_ms(10);/*让串口稳定*/
    
    while(Ms53l0m_RData_first(0xffff,IDSET_REG,2,&addr) && (i<10)) /*使用广播地址查询模块地址*/
    {
        delay_ms(200);
        i++;
    }
    Tof_Info2.id = addr; /*记录设备地址*/

   
    i=0;
    while(Ms53l0m_WData_first(Tof_Info2.id,WORKMODE_REG,MODBUS_WMODE) && (i<20)) /*设置MODBUS工作模式*/
    {
       delay_ms(200);
       i++;
    }
...
void Modbus_DataGet_first(void)
{
    ...
    res = Ms53l0m_WData_first(Tof_Info2.id,MEAUMODE_REG,HISPEED_DMODE); /*设置高速测量模式*/
    if(!res) ;
    else 
    {
