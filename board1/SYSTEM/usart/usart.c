...
void USART3_IRQHandler(void)                	//串口3 AT
{
	if(USART_GetITStatus(USART3, USART_IT_RXNE)!=RESET) //接收中断
	{
		uartRxFifo.message[uartRxFifo.len++] = USART3->DR;
		if(uartRxFifo.message[uartRxFifo.len-1] == '\n'||(uartRxFifo.len == UartFifo_MAXLEN))
		{
			uartRxFifo.message[uartRxFifo.len] = 0;
			uartRxFifo.over = 1;
		}
